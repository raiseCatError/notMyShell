...
### Added
- Smart output folding: finished commands no longer collapse just because they printed more than 10 lines. A deterministic completion-time policy folds only long, low-information successful output (repetition, progress rewrites, install/build/test chatter). Failures, output of 30 lines or less, diagnostics, stack traces, compiler/test errors, and diffs stay expanded, and output-is-the-result commands such as `git log`, `cat`, `rg`, and `ls` lean visible. A folded block keeps its first 3 and last 5 lines visible around `› N lines hidden · Ctrl+O`. It is presentation only: `/copy`, the session journal, `/resume`, and expansion keep the full output. `/settings` → Config → Output folding: Smart (default) / Never.
