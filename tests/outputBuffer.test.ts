...
test('historical prompt snapshots freeze their provider and original pigment through transcript restore', () => {
  const output = new OutputBuffer();
  const prompt = {provider: 'starship' as const, layout: 'twoLine' as const, cwd: '/tmp',
    segments: [{text: 'main', foreground: {red: 20, green: 190, blue: 80}, background: {red: 30, green: 170, blue: 70}, geometry: 'plain' as const}]};
  output.beginCommand('git status', ['❯ git status'], undefined, {cwd: '/tmp', prompt});
  prompt.segments[0]!.background.red = 240;
  output.complete(0);
  const first = output.wrapped(60).find(row => row.isHistoricalHeader)!;
  assert.match(first.plain, /main/u);
  assert.match(first.ansi, /38;2;\d+;\d+;\d+m/u);
  assert.equal(output.transcript().records[0]?.historicalContext?.prompt?.provider, 'starship');
  assert.equal(output.transcript().records[0]?.historicalContext?.prompt?.segments[0]?.background?.red, 30,
    'the submitted semantic snapshot is cloned');
  const restored = new OutputBuffer();
  restored.restoreTranscript(output.transcript());
  assert.deepEqual(restored.wrapped(60).find(row => row.isHistoricalHeader)?.plain, first.plain);
});
