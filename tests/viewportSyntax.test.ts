...
test('KNOWN COMMAND - submitted command retains ACCENT color', async () => {
...
test('UNKNOWN COMMAND - submitted command retains ERROR color', async () => {
    const app = new TerminalApp();
...
test('PIPELINE - echo "$HOME" | grep foo', async () => {
    const app = new TerminalApp();
...
test('MULTILINE - styles survive across multiline', async () => {
    const app = new TerminalApp();
    app['semanticService'].cache.set('git', 'executable');
...
test('ASYNC UPDATE - resolves and updates output buffer', async () => {
    const app = new TerminalApp();
    
    // We already proved this natively. We will bypass UI and just trigger update logic explicitly.
    Object.defineProperty(app.editor, 'text', { value: '', writable: true });
    app.editor.text = 'git status';
    
    // Submit! It will start as PRIMARY because cache is empty
    app['submit']('git status');
    
    // Fake the resolution callback
    app['semanticService'].cache.set('git', 'executable');
...
test('RAW OUTPUT - PTY output remains untouched', async () => {
    const app = new TerminalApp();
    submitCommand(app, 'git status');
...
test('COPY - /copy remains plain text', async () => {
    const app = new TerminalApp();
    app['semanticService'].cache.set('git', 'executable');
    submitCommand(app, 'git status');
