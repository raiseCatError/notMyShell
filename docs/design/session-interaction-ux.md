...
NMSh Native uses the ordered context modules (project, cwd, git branch, detected toolchains, and previous exit status). The `toolchain` module shows one segment per toolchain found by marker files in the cwd or repository root (Node `package.json`, Go `go.mod`, Python `pyproject.toml`/`requirements.txt`/`setup.py`/`Pipfile`/`.python-version`, Docker `Dockerfile`/compose files); saved configurations gain it at its default position. `palette` selects one of six role-based themes, each a complete map from segment role (project, cwd, git, each toolchain, success, failure) to colors: `lavender` (Lavender Native, default) uses a lavender/violet/iris family led by the NMSh brand lavender `#A67CF3`, alternating lightness so neighbours stay distinct; `brand` (Brand / Semantic) pairs `#A67CF3` project with slate path, charcoal git, and full Node green, Go cyan, Python yellow, and Docker blue; `semantic` (Soft Semantic) uses pastel versions with dark text; `cool` (Cool First) leads with periwinkle, slate, and teal; `warm` (Warm First) uses amber, sand, rust, and olive-adjacent tones; `grayscale` separates segments by graphite-to-silver luminance alone. Semantic themes keep the existing success/failure colors. Current Native settings include `startStyle` (`pointed` U+E0D7 opening, default, or `flat` square opening), `gapEnabled` (default `true`), and `endStyle` (`fadeWedge` by default, plus `wedge`, `fadeFlat`, and `flat`). `gap` remains the number of neutral cells when enabled; `/prompt` exposes Off (connected), Compact (`gap` 0: caps touch with no neutral cell), and Normal (`gap` 1). Today, `startStyle` also affects each independent internal segment opening. A planned geometry refinement will separate Start, Connector, Gap, and End so Start no longer controls internal connectors/openings. Prompt icons will also gain an independent On/Off preference. These follow-ups are tracked in #8; they are not implemented by the current settings. The existing `placement` key (`header` | `composer`) is exposed through the `/prompt` layout step together with `composerLayout`: "Two-line · prompt row is the divider" (`twoLine` + `header`: the prompt row's `─` fill is the composer's top edge), "Two-line · prompt inside bordered composer" (`twoLine` + `composer`: a separate border above the prompt row), and "One-line · prompt inline with input" (`oneLine`; stored placement is kept). Starship rows follow the same placement rule. `spacing` remains internal body padding. Explicit per-module colors still override any theme.
...
- Implemented onboarding configures prompt provider, composer layout, and Native appearance (theme, start style, gap, and ending), with real-renderer previews. The next onboarding scope is optional tool discovery/setup for zoxide, fzf, and Atuin, offering Recommended / Choose individually / Skip. Any installer that modifies the system requires explicit confirmation. Starship remains an optional prompt provider. NMSh replaces the UI roles of zsh-autosuggestions and zsh-syntax-highlighting; neither is a required dependency. This future work is tracked in #9.
- Native prompt themes currently style prompt roles. Broader NMSh presentation themes, including semantic editor/syntax colors where appropriate, remain future work in #58. The intended core directions are Lavender Native, Brand / Semantic, Cool First, Warm First, and Grayscale; Soft Semantic may be consolidated or retired after review.
- Real zsh/plugin lifecycle interoperability remains future work in #16. Safe configured-zsh completion and fzf-tab interoperability remain unresolved in #52; the documentation does not claim current native fzf-tab support.
