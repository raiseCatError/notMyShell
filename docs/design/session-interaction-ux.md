...
Separated context blocks own their terminating and entering Powerline caps, with neutral background in both cap cells and the configured gap. A two-line header ends its final block with a `▓▒░` density fade on neutral background and then one solid divider color. Geometry is shared between active and historical context; colors are not. Active project, cwd, git, status, and divider retain their distinct live colors, while historical headers use the muted lavender archive palette. If project and cwd display the same location, keep the project module and omit cwd.
...
Each fresh presentation begins with a compact welcome snapshot containing the compiled build identity, start cwd, and zsh label. The small cat and muted divider are rendered from that semantic snapshot as ordinary transcript rows, so they scroll away with history and can adapt to terminal width. `/clear` archives the old presentation and creates a new welcome at the current cwd without resetting zsh. `/resume` restores the archived snapshot if present and never injects another welcome. Welcome chrome is excluded from PTY output and `/copy`; it is separate from onboarding.
